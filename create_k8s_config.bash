#!/bin/bash
a=$(for i in {0..8}; do dig coord-$i.borg.trek | grep 192.168.180 | grep -Eo -e '(^[^\.]+)' -e '(192\.168\.180\.[0-9]+)' | tr '\n' ' ';echo ; done | grep -E '.+' | tr ' ' ':'  | sed 's/:$/,/g' | tr -d '\n' | head -c-1)
...
for f in coord-*; do cat $f >> hieradata/nodes/$(basename $f .yaml).borg.trek.yaml;done

# uncomment k8s
sed -i '/.*/s/^#//' site/profile/manifests/kubernetes/worker.pp
sed -i '/.*/s/^#//' site/profile/manifests/kubernetes/coordinator.pp
